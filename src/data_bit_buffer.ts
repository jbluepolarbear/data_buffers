...
export class DataBitBuffer {
    ...
    getBitAtOffset(offset: number) {
        ...
        let intValue = 0;
        if (bufferOffset < this.__view.getOffset() && this.__view._canReadAt(bufferOffset, size)) {
    ...
    setBitsAtOffset(offset: number, count: number, value: number) {
        for (let i = 0; i < count; ++i) {
            this.setBitAtOffset(offset + i, ((1 << (count - i - 1) >>> 0) & (value >>> 0)));
        }
    ...
    setUnsignedRangeBits(rangeBits: number[], value: number) {
        ...
        this.setIntRange(rangeMin, rangeMin + powerOf2(rangeBits[index]) - 1, value);
        return this;
    }
    ...
    getUnsignedRangeBits(rangeBits: number[]) {
        ...
        return this.getIntRange(rangeMin, rangeMin + powerOf2(rangeBits[index]) - 1);
    }
    
    _canRead(requestedSize: number) {
        if ((this.__offset >= this.__loadedLimit && this.__offset + requestedSize <= this.length()) || this.__offset + requestedSize <= this.__loadedLimit) {
            return true;
        }
    ...
    _canReadAt(offset: number, requestedSize: number) {
        if ((offset >= this.__loadedLimit && offset + requestedSize <= this.length()) || offset + requestedSize <= this.__loadedLimit) {
            return true;
        }
